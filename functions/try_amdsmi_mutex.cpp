...
void print_amdsmi_error(const char* function_name, amdsmi_status_t status) {
    std::lock_guard<std::mutex> lock(cout_mutex);
    std::cerr << "Error in " << function_name << ": " << status << std::endl;
}
...
void amdsmi_thread_func(int thread_id) {
    ...
    if (status != AMDSMI_STATUS_SUCCESS) {
        print_amdsmi_error("amdsmi_init", status);
        return;
    }
    {
        std::lock_guard<std::mutex> lock(cout_mutex);
    ...
    if (status != AMDSMI_STATUS_SUCCESS) {
        print_amdsmi_error("amdsmi_get_gpu_asic_info", status);
    }
    ...
    for (uint32_t device_id = 0; device_id < num_devices; device_id++) {
        ...
        if (status != AMDSMI_STATUS_SUCCESS) {
            print_amdsmi_error("amdsmi_get_processor_handles", status);
            continue;
        }
    ...
    for (const auto& handle : processor_handles) {
        ...
        if (status != AMDSMI_STATUS_SUCCESS) {
            print_amdsmi_error("amdsmi_get_gpu_board_info", status);
        }
        // Get GPU activity
        amdsmi_engine_usage_t activity;
        status = amdsmi_get_gpu_activity(handle, &activity);
        if (status != AMDSMI_STATUS_SUCCESS) {
            print_amdsmi_error("amdsmi_get_gpu_activity", status);
        }
        // Get VBIOS info
        amdsmi_vbios_info_t vbios_info;
        status = amdsmi_get_gpu_vbios_info(handle, &vbios_info);
        if (status != AMDSMI_STATUS_SUCCESS) {
            print_amdsmi_error("amdsmi_get_gpu_vbios_info", status);
        }
        // Get bad page info
